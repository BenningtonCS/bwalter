...
void insert(int n) {	
	...
	if(top == NULL) {
		...
		np -> next = top;		// set the new pointer to point top
		top = np;				// set top to point to the new pointer
	}
...
NodePtr search(NodePtr top, int n) {
	...
	while(np->data != n) {			// so long as the data is not n
		...
		if (np->next == top)		// if the next node is top
...
void printList(NodePtr top) {
	...
	while(np != NULL) {		// while np isn't the only node
