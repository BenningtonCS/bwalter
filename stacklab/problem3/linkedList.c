 ...
 *
 * implementation of a linked list of char*s
 */
 ...
 */
void insertHead(NodePtr *top, char* n) {
    NodePtr np = makeNode(n);	// create node
    np->next = *top;			// np points to the previously added node
	*top = np;					// top now points to np
}

/* Parameters: pointers to two node pointers, one being top and the other last
 * Returns: none
 * creates a new new and adds it to the tail of the linked list
 */
void insertTail(NodePtr *top, NodePtr* last,  char* n) {
