 ...
 * Puts data into the queue in a sorted fashion.
 */
int enqueue(Queue q, QueueData d) {
    ...
    // if the new node's data is higher than the first node
    if (np->data.val < q->head->data.val) { 
        np->next = q->head; 
        q->head = np;
        return 0;
    }
	...
	while (np->data.val >= tmp->data.val) {
